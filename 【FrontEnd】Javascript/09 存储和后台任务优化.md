...
> 本文涵盖三种浏览器后台机制：**Web Worker**（通用后台计算）、**WebAssembly**（高性能计算）、**Service Worker**（网络代理与缓存）。
...
Web Worker 是浏览器提供的**后台线程机制**，允许在独立线程中运行 JavaScript，从而避免阻塞主线程。
...
worker.onmessage = function (e) {
  console.log('结果:', e.data); // 收到 Worker 返回的数据
};
...
WebAssembly（Wasm）是一种**二进制字节码格式**，让 C/C++/Rust 等语言编译后的代码能在浏览器中以接近原生的速度运行，并与 JavaScript 协同工作。
...
Service Worker 是浏览器在**后台运行的独立线程**，作为页面与网络之间的代理，可拦截请求、缓存资源、推送通知，**生命周期独立于页面**。
...
Service Worker 的生命周期由浏览器托管，分为四个阶段：
