...
| 方法                                 | 适用场景   | 优点       | 缺点                                  |
| ---------------------------------- | ------ | -------- | ----------------------------------- |
| `typeof`                           | 基本类型检测 | 简单快速     | ⚠️ `null` 返回 `"object"`<br>无法区分对象类型 |
| `instanceof`                       | 引用类型检测 | 检测原型链    | ❌ 不能检测基本类型<br>❌ 不能跨 iframe          |
| `constructor`                      | 通用检测   | 可检测大部分类型 | ⚠️ 易被修改<br>❌ 不能检测 `null/undefined`  |
...
JavaScript 使用 IEEE 754 双精度浮点数标准，某些十进制小数无法精确表示为二进制。

### ✅ 解决方案

使用 `Number.EPSILON` 判断差值：
...
**学习建议**：重点关注标记 ⚠️ 和 💡 的内容，这些是易错点和最佳实践。
