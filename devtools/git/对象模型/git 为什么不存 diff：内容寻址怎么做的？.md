# git 为什么不存 diff：内容寻址怎么做的？

*类型：knowledge ｜ 难度：入门 ｜ 标签：Git、对象模型、SHA-1、快照、blob、tree、tag ｜ 更新：2026-09-10*
...
| 增量补丁模型 | 完整快照模型（Git） |
| --- | --- |
| 存储的是「变化」：每个版本 = 上一版 + 补丁 | 存储的是「状态」：每个版本 = 一棵完整的指针树 |
| 取旧版本要从头回放全部补丁，越老越慢 | 没变的文件引用旧对象，物理上不重复存储 |
| 当前版本快，历史版本越来越慢 | 取任何版本都是 O(1) 直接定位 |
| 历史链条一旦断一环，后面全废 | diff 只是展示用的临时计算，不参与存储 |
| 代表：早期 CVS / 直觉想象中的 Git | 对象不可变，损坏一环不影响其他版本 |
...
commit（第 2 次提交）
  ...
  │               └─ b.txt ──▶ blob b.txt（复用：与 v1 引用同一个对象）
  └─ parent ──▶ commit（第 1 次提交）
                  └─ tree ────▶ tree 根目录清单 v1
                                  ├─ a.txt ──▶ blob a.txt（旧内容）
                                  └─ b.txt ──▶ blob b.txt（同一个对象）
```
...
refactor: 事件循环笔记按最新规范重写——DoDont 陷阱对照、追问链面试模式……
```
...
> **记忆锚点：哈希即地址，快照即版本**
>
> 内容决定哈希，哈希决定地址：内容变 → 必然新对象；内容同 → 必然复用。每次提交是一棵完整的指针树（快照），没变的文件引用旧对象（物理增量）。版本管理的一切操作——存、取、分支、同步——都建立在这条之上。

## 一次 commit 之后发生了什么

把 `git commit` 放到显微镜下，它只做四步写操作，没有遍历、没有全局计算：

1. **存新 blob**：改动文件算哈希、zlib 压缩入库
2. **生成新 tree**：没变的条目直接复用旧 tree
3. **写新 commit**：指向新 tree + 上一个 commit
4. **改写分支**：分支文件指向新 commit（41 字节）
...
> 说明：「日常松散、定期打包」是设计意图——写入永不整理，整理交给 gc（存储篇细讲）。
...
**Q1：在一个已有提交的基础上，同一文件修改并提交 10 次，仓库里存了几份？**

*考察点：检验是否真正区分「快照模型」与「全量复制」——这是对象模型的第一道分水岭。注意口径：已有 1 个首次提交，再改 10 次。*
...
补充：Git 后台还会把松散对象打包成 packfile，对相似 blob 做字节级 delta 压缩——11 个版本在磁盘上可能只有 1 份基准 + 10 份增量。这是存储层的透明优化，不改变「每版一个对象」的逻辑模型。

**Q2：为什么 Git 改名文件不用重新存储，而某些系统「另存为」就翻倍占空间？**

*考察点：考察 blob「内容与名字分离」的设计是否真的理解，能否迁移到自己的系统设计。*
...
补充：这也解释了 Git 的 rename 检测为什么可靠：比较新旧 tree 时发现「同一个 blob 哈希换了名字挂载」，是精确匹配而非相似度猜测。

**Q3：两个内容完全相同的文件（比如各目录下的 LICENSE），Git 存几份？**

*考察点：检验「内容寻址 = 天然全局去重」这条性质的掌握程度，很多人答成两份。*
...
补充：推论：想验证「某个内容在历史里出现过没有」，把内容 hash-object 一遍、拿哈希去 cat-file 查即可，不需要遍历任何历史。

**Q4：SHA-1 哈希理论上会碰撞吗？Git 为什么敢把它当地址用？**

*考察点：进阶题。区分「背过 SHA-1 有碰撞新闻」和「理解 Git 工程上如何消化碰撞风险」的人。*
...
补充：对「寻址」而言，最重要的性质其实是单向性与确定性，而不是抗碰撞：地址由内容算出、同内容必得同地址，去重和寻址已经成立；碰撞只影响「不同内容被误判为相同」这一种极端情况。

**Q5：既然每次提交都是完整快照，为什么一个大仓库改 1000 次、提交 1000 次后，.git 目录并没有膨胀 1000 倍？**

*考察点：压轴题，检验能否把「快照 + 复用 + 打包」三层机制串成一条完整的存储链。*
...
补充：能说出「逻辑快照、物理增量」这个分层表述，再补一句 packfile 的 delta 只是存储优化、随时可无损还原成完整对象，就同时覆盖了正确性和工程直觉两层。

## 延伸阅读

- 前置：《git 的三个区是怎么分工的？》——add 把内容写进对象库：三区模型里的「对象库」正是本篇的主角。
- 关联：《reset --hard 丢弃的提交去哪了？》——「分支只是文件」配合本篇的对象图，让 rebase、reset、checkout 全部变成看图说话。
- 关联：《同一文件为什么有时冲突有时不冲突？》——对象模型之上的图算法：merge-base 与三方对比如何裁决冲突。
