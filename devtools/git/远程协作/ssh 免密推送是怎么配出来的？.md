...
*类型：practice ｜ 难度：入门 ｜ 标签：Git、SSH、密钥、ssh-agent、远程仓库 ｜ 更新：2026-09-10*

**Git 远程传输走 HTTPS 或 SSH 两种协议，SSH 用密钥对认证：私钥留在本机、公钥贴到平台，连接时靠「服务器出题、私钥签名」完成身份证明——全程不传输任何秘密，比密码安全且免输入。配置一次终身受益的四件事：生成 Ed25519 密钥对 → 公钥上传平台 → `~/.ssh/config` 写好主机别名与端口 → `git remote set-url` 切换协议。之后每次 push/pull 都不再需要任何身份输入。**

## 密钥对认证：不传秘密的身份证明

HTTPS 方式每次 push 都要身份证明（用户名 + token），SSH 方式把这件事一次性解决。核心是**非对称密钥对**：私钥（留在本机，谁都不给）+ 公钥（贴到 GitHub/GitLab 的设置页，随便谁看）。认证时服务器用你的公钥出一道「签名挑战」，本机私钥签出答案，服务器用公钥验证——**网络上只传输签名结果，私钥从不离开你的机器**，窃听者拿到全部流量也无法冒充你。
...
> **记忆锚点：私钥不出门，公钥随便贴**
>
> 密钥对认证的全部安全模型一句话：公钥是锁、私钥是钥匙——锁可以公开挂在网上，钥匙只在你机器里。判断文件：「.pub」结尾的是公钥可外传；不带后缀的是私钥，泄露 = 身份被盗，立刻在平台吊销并重新生成。
...
| --- | --- |
| 零配置开箱即用，clone 即可读 | 一次配置，之后 push/pull 全免认证 |
| 推送需 Personal Access Token（密码已废弃） | 私钥本机持有，不经过网络传输 |
| 走 443 端口，几乎不会被防火墙拦 | 多账号可用 config 精细分隔身份 |
| CI / 临时机器 / 只读场景更方便 | 默认 22 端口可能被封锁（用 443 备用入口） |
| 凭据管理依赖 credential helper | 日常开发机的主流选择 |

协议切换是纯本地操作，随时可逆：`git remote set-url origin git@github.com:user/repo.git` 切到 SSH，反向传 https:// 地址切回 HTTPS。remote 只是 `.git/config` 里的一行 URL（远程协作篇讲过 origin 的本质），换协议不产生任何对象迁移。
...
**Q1：为什么 SSH 密钥比「用户名 + 密码」安全？私钥不还是一样可能被偷吗？**

*考察点：热身题，检验对非对称认证模型的真正理解，而不是背「密钥更安全」。*
...
补充：硬件密钥（YubiKey）把私钥再收窄一层：私钥生成与签名全在硬件内完成，本机文件里只有句柄。

**Q2：ssh -T git@github.com 连接成功，但 git push 仍报 Permission denied，为什么？**

*考察点：分层排查题，检验能否区分「SSH 层认证」与「Git 层配置」两个独立环节。*
...
补充：GIT_SSH_COMMAND='ssh -v' git push 可以在 push 时直接打开 SSH 调试输出，不用单独跑 ssh 命令。

**Q3：公司网络封了 22 端口，git clone git@... 直接卡死，怎么办？**

*考察点：实战场景题，考察 443 备用入口这个经典解法是否知道。*
...
补充：如果 443 上的 SSH 也被深度包检测拦截，退路是把协议切成 HTTPS（remote set-url），用 PAT + credential helper——可用的出网端口决定协议选型，而不是反过来。

**Q4：一台机器上公司账号和个人账号都要用 GitHub，SSH 怎么配才不打架？**

*考察点：多账号是 SSH config 最高频的进阶需求，检验 Host 别名机制是否吃透。*
...
补充：全局 git config 的 user.name/user.email 也可以按目录覆盖（includeIf + gitdir 前缀），让提交作者信息和账号身份保持一致——身份认证（SSH）与提交署名（commit author）是两套独立系统，多账号场景两者都要理顺。

**Q5：.pub 结尾的文件是公钥可以随便发，那有人拿到我的私钥文件（没有 passphrase）会怎样？怎么补救？**

*考察点：安全压轴题，检验事故响应路径：密钥泄露的处置与密码泄露完全不同。*

拿到私钥 = 冒充你对该平台上所有仓库的读写（取决于该账号权限），且无需任何第二因素。补救动作有时序要求：①立即在平台（GitHub/GitLab）删除该公钥并生成新密钥对、上传新公钥——旧私钥立刻作废；②排查私钥可能泄露的途径（误提交进仓库、网盘、聊天记录发过）；③如果私钥曾被提交进任何仓库历史，按敏感信息泄露处理：filter-repo 清史 + 强推 + 所有协作者重克隆。预防永远便宜于补救：生成时设 passphrase，本机由 agent 缓存，兼顾安全与顺手。

补充：GitHub 的 Secret scanning 自 2023 年 11 月起把 OpenSSH 私钥列为 non-provider pattern：需在仓库/组织侧显式开启、且只产生告警——SSH key 没有吊销端点，「检测到即自动吊销」仅对提供 revoke API 的合作方 token 成立。所以别依赖平台兜底，自己的密钥自己盯。

## 免密推送配置自查

- ssh-keygen -t ed25519 生成密钥对——ed25519 是现代默认；用 rsa 要显式指定位数
- 公钥追加到目标机器 ~/.ssh/authorized_keys——是追加（>>）不是覆盖（>）
- ~/.ssh 目录 700、authorized_keys 600——权限过松时 sshd 直接拒收公钥
- git 推送前先用裸 ssh 验证链路——ssh -T 能通，git@ 的推送才有基础
- 首次连接核对主机指纹后再 yes——防中间人：指纹要对得上服务端提供的值

## 延伸阅读

- 前置：《SSH 是怎么保证远程登录安全的？》——免密推送用的是 SSH 公钥认证：握手与密钥对的机制是那几条配置命令的地基。
- 关联：《SSH 是怎么保证远程登录安全的？》——传输层之下：sshd、握手七步与签名挑战模型的完整拆解。
- 关联：《origin/main 是远程上的分支吗？》——配好的通道上传输的是什么：fetch/push 协议与远程书签机制。
