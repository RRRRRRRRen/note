...
*类型：knowledge ｜ 难度：入门 ｜ 标签：Git、分支、HEAD、reflog、指针、detached HEAD ｜ 更新：2026-09-10*
...
> **记忆锚点：分支 = 会移动的指针**
>
> 分支的物理实体是 `.git/refs/heads/` 下的一个文件，内容为一行 commit 哈希。提交时 Git 做的唯一「分支操作」就是改写这个文件——「分支前进了」是结果，不是过程。理解了这一点，fast-forward、branch -d 的安全性、分支对比（其实就是比哈希）全都顺理成章。
...
1. **读 HEAD**：ref: refs/heads/main
2. **读 main 文件**：拿到当前提交哈希
3. **写新 commit**：parent = 刚拿到的哈希
4. **改写 main**：指向新 commit，分支前进
5. **追加 reflog**：HEAD@{1} → HEAD@{0}
...
后果在你切走的那一刻发生：HEAD 指向别的分支后，刚才那个提交不在任何分支的引用链上——`git log` 里看不到了。很多人在这里「丢了代码」。但结合上一段就知道：**对象本体还在对象库里，reflog 里也记着这一笔**，找回只是从 reflog 里把哈希抄出来、建个分支指过去的事。真正会丢数据的场景只有一种：放着不管超过 reflog 过期时间，再被 GC 修剪（见「存储与回收」篇）。
...
> 说明：切走之后新提交不在任何分支链上，git log 自然看不到——但对象还在，别慌，也别急着重做。（输出为节选，警告全文会再解释 stash 与 -c 建分支两条出路。）
...
三个视角对照着读：`git log` 看的是「从分支可达的提交」；`git reflog` 看的是「HEAD 走过的路」（不管可不可达）；`git fsck --unreachable` 看的是「对象库里所有没人指的对象」。c2 在第一个视角消失、在后两个视角都在。真正的物理删除要等两件事同时发生：reflog 条目过期（可达记录默认 90 天 / 不可达记录默认 30 天）+ GC 修剪可达性。

```text
GC 可达性入口 / reachability

git gc 修剪的「存活名单」从三个入口出发遍历：
  ├─ 入口①：所有引用（分支 / 标签 / 远程书签）
  ├─ 入口②：所有未过期 reflog 条目（可达 90 天 / 不可达 30 天）
  └─ 入口③：当前 index（暂存区）
        │
        ▼
   三个入口都走不到的对象 ──▶ 才会物理删除
...
| ~ 与 ^（结构坐标） | @{n}（时间坐标） |
| --- | --- |
| 在提交图上按形状移动 | 在 reflog 上按时间回退 |
| HEAD~3 = 沿第一 parent 上溯 3 代 | HEAD@{1} = 上一次移动前的位置 |
| HEAD^2 = merge 的第二个 parent | main@{yesterday} 也合法 |
| 回答「那个提交的祖先是谁」 | 回答「我昨天指过谁」——找回丢失提交的主力 |

> **记忆锚点：reset --hard 之后先看 reflog**
>
> 任何「提交不见了」的事故，恢复口诀：`git reflog` 找到事故前的哈希 → `git branch rescue <哈希>`（或直接 reset 回去）→ 检查无误再清理。reflog 是本地保险，push/clone 不携带，所以「找回」只能在出事的这台机器上做。
...
**Q1：为什么 Git 开分支快到感觉不到延迟？**

*考察点：热身题，检验「分支 = 指针文件」是不是真的内化，而不是背一句口号。*

因为创建分支的物理动作是：新建一个 41 字节的文件，内容写一行哈希。没有代码拷贝、没有目录复制、没有索引重建——对比一下「复制整个项目目录」的开销就能理解量级差异。切换分支的开销另算（需要物化差异文件），但「开」这个动作本身永远是毫秒级。

补充：由此能推出分支管理的正确姿势：分支便宜到可以按想法随手开（一个实验一个分支），贵的从来不是分支数量，而是长期不合并导致的冲突面积。

**Q2：git checkout main 之后，之前 detached 状态下做的提交还在吗？去哪找？**

*考察点：「丢代码」是 detached HEAD 最高频的事故，考察是否掌握 reflog 这条恢复路径。*
...
补充：git fsck --unreachable 能列出所有不可达对象，是 reflog 也被清掉之后的最后手段；日常用 reflog 就够，因为 reflog 里连「你切过去」这个动作都记着。

**Q3：git reset --hard 和 git checkout 切分支，本质区别是什么？**

*考察点：两者都是「移动指针」，区分不开的人会把 reset 用在共享分支上闯祸。*
...
补充：reset --soft/--mixed/--hard 三档的区别只是「分支挪走之后，工作区和 index 要不要跟着动」：soft 都不动、mixed 动 index、hard 全部对齐——对象库里的旧提交在任何档位下都不删，reflog 都能救。

**Q4：branch -d 和 branch -D 的区别，从引用模型怎么解释？**

*考察点：考察能否用「可达性」解释命令的安全设计，而不是死记 -d 安全 -D 强制。*
...
补充：tag 和 branch 的唯一区别也在这里：tag 创建后从不移动。但「删 tag 永远安全」是误解——tag 没有 reflog，如果某个提交只被这个 tag 可达（没有任何分支指着），删掉 tag 后它立刻失联，reflog 里捞不到，只能靠 git fsck --unreachable 这类对象级扫描找回。

**Q5：reflog 会不会无限膨胀？它和对象库的 GC 是什么关系？**

*考察点：压轴题，把 reflog 的「保险」属性和 GC 的「回收」属性拼成完整的生命周期图。*
...
补充：gc.reflogExpire / gc.reflogExpireUnreachable 两个配置可以调整保留期；团队规范里如果有人经常 rebase 共享分支，可以把窗口调长当作事故保险。

## 延伸阅读

- 前置：《git 为什么不存 diff：内容寻址怎么做的？》——分支只是指向提交的引用：引用指向的正是内容寻址仓库里的提交对象。
- 关联：《git 的三个区是怎么分工的？》——三区坐标系里 status/diff 的方向语义，日常命令的下一层地基。
- 关联：《restore、reset、revert 怎么选？》——把本篇的指针模型用起来：撤销四场景的路由与 reset 三档实验。
