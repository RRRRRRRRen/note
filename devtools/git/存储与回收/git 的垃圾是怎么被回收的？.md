...
*类型：knowledge ｜ 难度：入门 ｜ 标签：Git、GC、packfile、delta 压缩、松散对象、可达性 ｜ 更新：2026-09-10*
...
把引用系统篇的 reflog 保险串起来，一次「事故提交」的完整生命周期是：被 reset 抛弃（分支不再指着它）→ 成为不可达，reflog 里的记录默认再保 30 天（`gc.reflogExpireUnreachable`；仍被引用指着的提交走 90 天档）→ 条目过期 → 下一次 gc 物理删除。**双重条件都满足才会真的丢**——这就是「Git 里很难真正丢数据」的精确含义。

> **记忆锚点：逻辑删除 ≠ 物理删除**
>
> branch -d、reset、rebase 抛弃的提交，只是从引用链上摘下来；只要 reflog 未过期，随时能救回来。真正的删除 = reflog 过期 + gc 修剪，两个条件缺一不可。反过来，想让机密文件彻底从仓库消失，光 revert 不够——历史里的旧 blob 依然可达，必须改写历史（filter-repo）再让所有克隆重新同步。
...
| --- | --- |
| 工作区只物化你需要的子目录 | 大文件出库（LFS / 对象存储，见大文件专篇） |
| 对象库仍然完整（历史都在） | 构建产物绝不入库（.gitignore 前置） |
| checkout 只写选中的那部分文件 | 一个仓库一个领域，避免万物 monorepo |
| Chromium / Android 团队的日常形态 | 历史臃肿后无法自愈——预防远便宜于治理 |
| 配 partial clone：连用不到的 blob 都不下载 | git filter-repo 是事后手术，代价是全团队重克隆 |
...
解法是把大文件请出对象库——Git LFS 的指针文件机制、数据集场景的 DVC、clone 侧的 `--filter` 部分克隆，以及已经入库后的 filter-repo 清史手术，这些是另一条完整的故事线，全部在大文件专篇展开。本篇剩下的部分回答纯 Git 侧的问题：对象何时打包、何时被物理删除。

## 追问链

**Q1：git gc 会不会把我没提交的工作区改动删掉？**

*考察点：热身题，检验对 GC 作用范围的理解——它管对象库，不管工作区。*
...
补充：唯一理论例外：git stash 的悬挂暂存超过默认保留期后会被清——stash 本质是 commit 对象，可达性靠 stash 引用维持，drop 掉又过期的 stash 才真正无主。

**Q2：为什么 Git 对文本文件的存储效率这么高？说出一层以上的原因。**

*考察点：检验能否分层作答：对象级去重与包级 delta 是两个独立机制。*
...
补充：zlib 还会吃掉一层：源码文本压缩比通常 2-4 倍，这是与内容无关的通用压缩，打包前后都在生效。

**Q3：频繁切换分支对 CPU 和磁盘压力大吗？成本到底花在哪？**

*考察点：检验能否推翻「全量重建」的直觉，建立「差异物化 + 系统调用主导」的正确成本模型。*
...
补充：极端案例（千万文件级 monorepo）的官方解法 sparse-checkout + partial clone 全是「减少碰文件系统的数量」，从侧面证明瓶颈永远在文件系统操作而不在解压。

**Q4：误提交了一个带密码的文件后来删掉了，仓库还安全吗？**

*考察点：安全题。检验「逻辑删除 vs 物理删除」的理解能否落地到安全事故处置。*
...
补充：BFG Repo-Cleaner 是 filter-repo 的替代品；GitHub 对敏感数据还有官方协助渠道（撤下缓存视图）。事故处置的优先级永远是：先作废凭据，再清理历史——历史清理慢一步没关系，密码泄露多一刻都是事故。

## 延伸阅读

- 前置：《git 为什么不存 diff：内容寻址怎么做的？》——垃圾回收回收的是「不可达对象」：可达性由引用关系决定，先懂对象模型。
- 关联：《仓库为什么被几张大文件撑爆？》——delta 救不了的二进制大文件：LFS 指针机制、DVC、partial clone 与清史手术。
- 关联：《git 为什么不存 diff：内容寻址怎么做的？》——GC 修剪与打包的对象从哪来：blob/tree/commit/tag 的物理形态。
