...
*类型：knowledge ｜ 难度：进阶 ｜ 标签：Git、Git LFS、大文件、二进制、partial clone、DVC ｜ 更新：2026-09-10*
...
内容寻址的去重靠「内容相同 → 哈希相同」，但二进制大文件的每个新版本都是不同的内容，必然生成新 blob——**逻辑快照、物理增量在它身上失效**。100 MB 的模型文件改 50 次 ≈ 5 GB 进包；而且对象库的同步原则是「远端报出它有的哈希，缺什么传什么」（fetch-pull 篇讲过协商机制），**去重救不了「别人根本没有」的对象**——每个新克隆、每个 CI 环境、每个新同事，都得把全部 5 GB 拉下来。仓库一旦背上这份历史，它无法自愈：后续的删除只影响之后的版本，历史里的旧 blob 依旧可达（GC 篇的可达性法则）。

```text
同一大文件两种管理方式的克隆体积（数量级直觉，非精确值）

直接进 Git（50 个版本）    │████████████████████████████████  5000 MB
Git LFS（按需拉取当前版）  │█                                    100 MB
```
...
> 说明：删除只改「之后的版本」，可达性不变——这是 GC 篇「逻辑删除 ≠ 物理删除」在大文件场景的直接推论。
...
**Q1：Git 为什么没法像文本那样压缩二进制文件的历史版本？**

*考察点：热身题，检验能否把「delta 压缩是字节级」这个认知精确化，而不是笼统说「二进制压不动」。*
...
补充：由此能推出一个工程技巧：必须入库的数值数据用未压缩或列存格式（如未压缩 CSV/Parquet）比 zip 打包后入库更省——前者能吃到 delta，后者每版全量。

**Q2：Git LFS 为什么能解决大文件问题？它的本质是什么？**

*考察点：检验能否识别 LFS 与 Git 对象库的同构性——这是整条内容寻址主线的回响。*
...
补充：本质层面：LFS 就是一个迷你版内容寻址数据库——与 .git/objects 同构（哈希即地址、指针引用、天然去重），只是存储后端从本地 packfile 换成了 HTTP 服务。「内容寻址 + 指针」是可迁移的架构模式：DVC、Docker 镜像层、CAS 存储、IPFS 都在用同一套思想。

**Q3：CI 上 clone 一切正常，构建时却报「模型文件只有 130 字节」，怎么回事？**

*考察点：实战场景题，LFS 在 CI 的翻车是最高频的线上事故，考察对 smudge 过滤器时机的理解。*
...
补充：git lfs ls-files 能列出哪些文件是 LFS 指针、哪些已检出（前面带 - 或 * 标记），是排查这类问题最快的一条命令。

**Q4：filter-repo 清完历史、强推之后，团队里每个协作者要做什么？为什么不能直接 pull？**

*考察点：压轴题，检验「改写历史 = 全员重克隆」背后的对象模型原因，以及事故处置的完整性。*
...
补充：GitHub 上被清除文件的 PR 引用、缓存视图可能残留（官方有协助撤下的渠道）——托管平台的边缘缓存是 filter-repo 管不到的盲区。

**Q5：partial clone 既然是官方机制，为什么不默认开启？它和 LFS 是竞争关系吗？**

*考察点：进阶辨析题，考察对两种方案定位差异的理解——答「用 LFS 就行」说明没分清要解决的问题。*
...
补充：promisor remote 是 partial clone 的底层概念：仓库声明「缺的对象可以找某个 remote 要」，补拉动作对用户透明，git log --filter=blob:none 还能只列提交不展开文件。

## 延伸阅读

- 前置：《git 的垃圾是怎么被回收的？》——本篇的成本推导建立在「对象按内容寻址 + packfile delta 压缩 + 同步按哈希集合求差」三层机制上。
- 前置：《origin/main 是远程上的分支吗？》——同步按哈希集合求差是克隆税的由来。
- 关联：《git 的垃圾是怎么被回收的？》——可达性法则与 packfile 机制的本篇源头：为什么「删了」不等于「没了」。
- 关联：《origin/main 是远程上的分支吗？》——克隆税的由来：同步按哈希集合求差，别人没有的对象必须传。
