# restore、reset、revert 怎么选？

*类型：practice ｜ 难度：入门 ｜ 标签：Git、restore、reset、revert、stash、撤销 ｜ 更新：2026-09-10*
...
reset 的核心动作只有一个：**把当前分支指针改写到目标提交**（引用系统篇讲过，分支只是一个 41 字节的指针文件，这正是它危险的原因——共享分支被 reset 等于改写公共历史）。三档的区别只在于「指针挪走之后，index 和工作区跟不跟着对齐」。下面用四个提交的临时仓库把三档一次性真实走完：
...
1. **--soft**：只动分支指针，index 与工作区原样
2. **--mixed**：指针 + 重置 index（默认）
3. **--hard**：指针 + index + 工作区全对齐
...
> **记忆锚点：reset 动的是指针，不是对象**
>
> 任何档位的 reset 都不删除提交对象，只是把分支文件改写到目标提交。旧提交从分支链上「摘下来」，靠 reflog 维持可达；真正物理消失要等 reflog 过期 + gc 修剪两个条件同时满足。
...
| --- | --- |
| 生成新提交，内容 = 目标提交的反向 diff | 分支直接改指到目标提交 |
| 不改写任何已有提交，无分叉风险 | 目标之后的提交脱离分支链（reflog 可救） |
| 已 push / 共享分支的唯一推荐 | 仅限未 push 的本地提交 |
| 撤销合并时需要 -m 指定保留哪条主线 | 配合三档决定工作区/index 怎么清理 |

撤销 merge 提交时 `git revert -m 1 <哈希>` 的原理：普通提交的撤销方向唯一（反向 diff 即可），而 merge commit 有两个 parent，Git 无法自行判断「抵消哪条线、保留哪条线」——`-m 1` 声明「沿第一个 parent（通常是合入时的主线）的方向保留」，merge 带进来的那批改动全部被反向抵消。这里「parent 的结构语义」在合并篇有完整展开。
...
**Q1：git commit --amend 到底做了什么？已 push 的提交还能 amend 吗？**

*考察点：热身题，检验「amend = 替换而非修改」这个对象模型认知。*
...
补充：amend 前忘 add 文件是经典场景：先 git add 漏的文件、再 amend --no-edit，一次提交补全，不用 cancel 重提。

**Q2：git reset --hard 把不想删的改动删了，还能救吗？**

*考察点：高频事故题，检验 reflog 恢复路径是否形成条件反射。*
...
补充：IDE 的 Local History、编辑器 undo 栈是未 add 内容的最后一线希望——但这属于编辑器的仁慈，不是 Git 的承诺。

**Q3：reset --soft、--mixed、--hard 各适合什么场景？为什么 --hard 最危险？**

*考察点：检验三档差异是否精确到「清理哪几区」，而不是背口诀。*
...
补充：git reset 等价于 reset --mixed HEAD，只重置 index 不动提交——这是「全部撤下草稿」的快捷方式，与 restore --staged 的多文件版对应。

**Q4：revert 一个 merge commit 为什么会报错？-m 参数在选什么？**

*考察点：进阶题，检验 merge commit 双 parent 模型在撤销场景的应用。*
...
补充：revert 掉 merge 之后又想重新合入这个分支，直接 merge 会「看起来无变化」（历史里已有这些提交）——需要 revert 那个 revert，或 rebase 生成新哈希再合。

**Q5：stash 和 branch 都能「先存改动再切走」，怎么选？**

*考察点：压轴题，检验能否按时间尺度与语义给两个工具划界。*
...
补充：git stash branch <新分支> 能把指定 stash 直接变成新分支上的改动——当临时周转发现东西值得长做时，这就是 stash 到工作线的转正通道。
...
- 前置：《git 的三个区是怎么分工的？》——撤销的全部路由都建立在三区模型上。
- 前置：《reset --hard 丢弃的提交去哪了？》——reset 的安全性问题要到「分支只是指针文件」那一层才看得透。
- 关联：《reset --hard 丢弃的提交去哪了？》——reflog 与 GC 的完整生命周期，reset 后悔药的物理基础与时间窗。
- 关联：《同一文件为什么有时冲突有时不冲突？》——revert -m 背后的 merge commit 双 parent 结构语义。
