...
*类型：knowledge ｜ 难度：入门 ｜ 标签：制品仓库、Nexus、Harbor、Verdaccio、选型 ｜ 更新：2026-09-10*
...
选型前先知道私有仓库是什么、三种仓库角色怎么分工——本篇只回答「挑哪个」。（前置篇：《Nexus 是什么：为什么公司都要自建制品仓库？》）
...
- 选型只问两件事：①要管几种制品？决定「专项工具还是全家桶」；②安全要多深？决定「开源 OSS 够不够、要不要 Harbor 或商业版」
- 两问的答案直接落在工具格子里——不看出身，不看名气
...
| 错误做法 | 正确做法 |
| --- | --- |
| 大厂都用 Artifactory，我们也上 | 先盘点：npm？镜像？Maven？ |
| 企业级平台的运维与授权成本对小团队是反噬——它在为合规审计、多站点复制这些你还没有的问题付费 | 按制品种类选：单一生态用专项工具，多生态用 Nexus 一台全包起步，规模到了再加码 |

registry:2 当企业仓库：

| 错误做法 | 正确做法 |
| --- | --- |
| docker run -d -p 5000:5000 registry:2 | 企业及格线 = 权限 + 可视 + 清理 |
| 无 UI、无权限、无清理策略——镜像谁都能推、磁盘满了没人知道。它只适合个人实验 | 多用户、多项目环境至少选带认证、界面与磁盘治理的工具（Nexus / Harbor） |

> **警告：有仓库 ≠ 有安全。**Harbor 的扫描、签名、准入是**能力上限**，不是装完默认生效——策略不配照样裸奔；Nexus OSS 同样不带镜像漏洞扫描。把「我们有自己的仓库了」当成安全达标，是选型环节最后的错觉。

## 追问链

**追问 1：Verdaccio 零配置又免费，为什么不能顺手拿来管 Docker 镜像？**

> 考察点：热身题：确认「工具绑定生态协议」这个根子——不是功能多少的问题，是协议实现的问题。
...
- 再进一步：OCI（Open Container Initiative）distribution 规范是镜像仓库的通用语言：Harbor、registry:2、Nexus 的 Docker 仓库、云厂商镜像服务都说这门语言，所以 docker 客户端可以无差别对接

**追问 2：同样都「有 UI、有权限」，Harbor 和 Nexus 的能力分界在哪？**

> 考察点：考察能否超越「都是仓库」的表面相似，说出两者各自的最优区间——这是两轴决策法能不能落地的关键。
...
- 再进一步：Nexus 的镜像扫描在商业版里（Pro 层），开源团队常用组合是 Nexus 管包 + 外挂 trivy 在 CI 里扫镜像——用流水线补齐 Harbor 式能力

**追问 3：为什么很多大厂让 Harbor 和 Artifactory 并存，而不是统一成一家？**

> 考察点：收束题：考察能否理解「组合优于单一」背后的决策逻辑——统一入口的收益 vs 单点工具的深度损失。
...
- 再进一步：中小团队的对应折中是 Nexus 一台全包：放弃 Harbor 的安全纵深，换取「只运维一套」的简单——决策轴没变，只是权重不同

**追问 4：团队已用 Nexus OSS 管镜像，现在要扫描、签名、准入——从 Nexus 迁到 Harbor 的稳妥路径是什么？**

> 考察点：实战收束：选型不是一次性决策——考察能否给出带迁移成本的演进路径，而不是「推倒重来」或「永远凑合」两个极端。
...
- 再进一步：镜像搬运优先用 skopeo copy 而非 pull→push：不经本地 Docker 守护进程、不落盘，且完整保留架构信息与 digest——跨仓库同步是它的本职

## 下一步去哪

选型至此收口。最好的验证是动手：把 Nexus 用 `docker run` 跑起来（官方镜像 `sonatype/nexus3`，数据挂 volume），建一个 hosted、一个 proxy、一个 group，把自己的前端镜像 push/pull 走一遍——三角色怎么协作、通道怎么跑，两个姊妹篇正好是操作时的对照手册。

## 延伸阅读

- 《Nexus 是什么：为什么公司都要自建制品仓库？》：选出来的这台服务器内部怎么运转：proxy / hosted / group 三角色协作拓扑
- 《镜像怎么从构建机到部署机？》：仓库在部署链路里的位置：save/load 与 push/pull 两条通道的取舍
