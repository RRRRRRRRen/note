# references 和 tsc -b 解决什么？

*类型：knowledge ｜ 难度：高级 ｜ 标签：tsc -b、project references、tsbuildinfo、monorepo ｜ 更新：2026-09-10*

**当一份 tsconfig 装不下两套语义（浏览器代码 vs Node 配置）、或一个 monorepo 里多个包互相依赖时，`references` 负责声明「谁依赖谁」，`tsc -b` 负责按依赖图的拓扑序增量构建。机制核心是契约墙：检查依赖方时不重查被依赖方的源码，只读它的 .d.ts 声明面——这是性能的秘密，也是声明文件之所以必须存在的原因之一。没有 references 时，`tsc -b` 退化为带增量的 tsc——本仓库的现状，也是日后拆分预留的门。**
...
```text
拆分蓝图 / app & node（蓝图，非本仓库现状——本仓库是单 tsconfig）

tsconfig.json（空壳：files: [] + references）
  ├─ reference ─► tsconfig.app.json：src/**，lib: DOM，types: vite/client
  │                   ┄► 各自独立的 tsbuildinfo 增量缓存
  └─ reference ─► tsconfig.node.json：vite.config + plugins，lib: ES，types: node
                      ┄► 各自独立的 tsbuildinfo 增量缓存
```
...
> **说明：单仓库拆分的完整三文件形态**
> 根 tsconfig 只做壳（`files: []` 加 references），真实配置分居 app/node 两个子文件——build script 一个字不用改，`tsc -b` 自动按图编排。
...
本仓库没有 references，所以 `tsc -b` 的实际效果是「带增量的 tsc」：全项目查一遍、写 tsconfig.tsbuildinfo、报错即中断——与裸 `tsc --noEmit` 的差别主要就是增量缓存。顺手的纪律提醒：tsbuildinfo 是本地缓存（内容含机器相关路径），应该 gitignore；本仓库它目前被 track，值得处理。

缓存纪律的机制根源在 tsbuildinfo 的内容：它记录「上次检查时的文件集合、内容指纹与配置状态」，构建时先对账再决定查谁。两个推论：一是配置指纹参与对账——改任何 compilerOptions 让整个缓存作废是设计行为而非 bug，代价是下次全量；二是「改了代码却没重查」的反向症状多发生在契约墙场景——下游认的是上游的**声明面**，上游内部改动不触碰声明面就不触发下游重查，这是增量性的来源；只有当缓存状态与磁盘真实状态脱节时，才需要 `--clean` 强制对账。

## 四个高频坑

**include 边界互斥（disjoint includes）**
...
- 契约墙只认声明面变化；缓存状态与磁盘脱节时，改再多源码也不会触发重查。
...
- 先清缓存再构建——排除缓存失灵之后，才轮到怀疑项目配置与依赖关系。

第四个坑在编辑器侧：**项目结构大改后 tsserver 的内存路由可能滞后**（文件换了所属子项目，红线还是按旧 project 给的）——Restart TS Server 重建路由即可，机理与「编辑器的 TS 智能是怎么来的？」一篇讲的常驻 program 一致。

## 追问链

五问按「是什么 → 墙 → 编排 → 治理」推进，后两问偏架构判断。

**1. tsc -b 和裸 tsc --noEmit 差在哪？**

考察点：确认 build 模式的最小语义——很多人以为 -b 只是 --noEmit 的别名。
...
考察点：机制价值题——答「少查文件」只对一半，关键是把重查范围从实现面缩到声明面。
...
**3. composite: true 强制的是什么？为什么契约墙需要它？**

考察点：机制题——把「墙需要原料」这个比喻落回具体选项。
...
考察点：把 tsc 的编排语义延伸到 tsserver——references 不只是构建期概念。
...
**5. 什么信号出现时，才值得把单 tsconfig 拆成 references？**

考察点：架构判断题——references 是解药也是复杂度，筛掉「为拆而拆」的人。

两个信号：一是「一份 compilerOptions 装不下两个世界」（vite.config 需要 node types 而 src 需要 DOM，互相污染检查语义）；二是多包仓库出现明确的依赖方向与增量痛点（改 core 全仓重查）。只嫌一个 tsconfig 行数多不是理由——拆分引入 references 编排的心智成本，要先有真实的痛。

补充：本仓库的判断——单配置目前能用（vite.config 在 DOM lib 下检查是纯度问题不是正确性问题），拆分收益中等——等插件目录变大、或 Node 侧 API 用深了再拆，三文件骨架随时可套。
...
- tsconfig 的一份配置到底谁在读？——「一份配置装不下两个世界」是拆分的动机——本篇给出标准的拆法与编排机制。增量缓存（tsbuildinfo）的思想与 HMR 同源。

后续阅读：

- .d.ts 声明文件到底解决什么问题？——契约墙的原料就是它——声明文件的四个存在理由与信任模型。
- 编辑器和构建的类型检查为什么会不一致？——拆分之后 include 边界与推断项目的关系——漂移治理的另一半。
