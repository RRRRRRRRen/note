...
*类型：knowledge ｜ 难度：入门 ｜ 标签：浏览器、Ajax、XMLHttpRequest、网络请求*

**Ajax 是「不重载整页、与服务器交换数据并局部更新」的技术总称，XMLHttpRequest（XHR）是其经典实现：流程固定为 `open()` → `setRequestHeader()` → `send()` → 监听 `readyState`，`readyState === 4 && status === 200` 即成功。工程要点在三件事——上传进度走 `xhr.upload.onprogress`、取消请求用 `AbortController`（fetch 同样适用）、失败重试配指数退避并设超时。现代开发中 JSON 已取代 XML，fetch 与 Axios 是日常首选，但 XHR 的事件模型与状态机仍是理解请求生命周期的底座。**
...
xhr.onreadystatechange = () => {
  // readyState 4 = 响应全部接收，status 200 = 服务端成功
...
function sendGetRequest(url, params) {
  // 参数序列化为查询串，encodeURIComponent 防止特殊字符破坏 URL
...
document.querySelector('#input-file').addEventListener('change', function (e) {
  ...
  // FormData 自动按 multipart/form-data 编码，无需手动设置请求头
  ...
  // upload.onprogress 提供上传进度（下载进度在 xhr.onprogress）
...
```js
// signal 与 controller 关联，abort 时请求被取消并抛出 AbortError
