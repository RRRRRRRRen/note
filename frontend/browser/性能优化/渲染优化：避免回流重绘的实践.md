...
*类型：practice ｜ 难度：进阶 ｜ 标签：渲染优化、回流、重绘、合成层、性能优化*

**渲染优化的实践主线是「降低改动的成本档位、减少改动的次数」：回流（重建渲染树）代价高于重绘（仅重填像素），两者都远高于合成层更新（GPU 直改，完全绕过回流重绘）。落地手段依次是——避免写后立即读的强制同步布局、批量读再批量写、合并样式修改并用 DocumentFragment 批量插入、动画只用 transform/opacity 并按需提升合成层、简化高开销 CSS 效果、用虚拟滚动和延迟加载把 DOM 数量与时机关进可控范围。**
...
```js
el.style.width = '100px';          // 写：把布局树弄脏
const height = el.offsetHeight;    // 读：脏树 → 强制 layout
```

循环读写（最危险）：

```js
// 每轮「写 → 读」交错，一帧之内 N 个节点触发 N 次完整布局
...
```js
// 1. 先读取所有值（只触发一次 layout，后续读的都是干净树）
...
```css
/* 不推荐：box-shadow 动画持续触发重绘 */
.card { transition: box-shadow 0.3s ease; }

/* 推荐：迁移到 transform，只走合成 */
.card {
  will-change: transform;
