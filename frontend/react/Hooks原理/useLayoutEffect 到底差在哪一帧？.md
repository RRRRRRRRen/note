...
*类型：knowledge ｜ 难度：进阶 ｜ 标签：React、Hooks、useEffect、渲染 ｜ 更新：2026-09-10*

**差别在一道**浏览器绘制（paint）**的分界线：两者都在 commit 阶段、真实 DOM 更新之后执行，但 `useLayoutEffect` 在 **paint 之前同步执行**——函数体跑多久，用户就多久看不到新界面；`useEffect` 被标成 passive，交给调度器排在 **paint 之后异步执行**。所以九成场景该用 `useEffect`；唯一必须用 `useLayoutEffect` 的正当场景是「测量 DOM 并在用户看到之前调整」——比如 tooltip 定位，用错会闪烁。**

## 前置篇说明

- 前置篇：《连续 setState 为什么只加一次：批处理》（hooks/hooks-render）
- 本篇接着渲染往下走：批处理解决了「什么时候渲染」，这篇回答「渲染提交之后，effects 在帧里的哪个时点跑」。
...
补充说明：移除的是噪音，不是语义：无论哪个版本，服务端都不会执行任何 effect。

## 边界与陷阱

除「测量放错档」外，这一对 Hook 还有三个高频坑：在 useLayoutEffect 里做重活（阻塞被自己放大成掉帧）、在 layoutEffect 里无条件 setState（paint 前同步重渲染，阻塞时间翻倍，还容易写出无限循环）、以及 SSR 首屏空窗（上一节）。前两个对照：

反例 vs 正例一（测量时机 / measure before paint）——passive 执行时首帧已上屏，测量结果只能修正「下一帧」——中间帧就是闪烁本身：

```typescript
// 反例：测量放 useEffect：paint 已经发生
useEffect(() => {
...
```typescript
// 正例：测量 + 调整放 useLayoutEffect：paint 前
useLayoutEffect(() => {
  const r = ref.current!.getBoundingClientRect();
...
补充说明：layoutEffect 里 setState 会在 paint 前同步重渲染并提交——正是「画之前改好」的实现机制。

反例 vs 正例二（layoutEffect 的开销 / keep it light）——阻塞绘制的工作全挤在 paint 前：重计算 + 布局抖动让每一帧都变长：
...
补充说明：默认 useEffect；layoutEffect 只留轻量的测量与调整，读写集中一次完成。

## 经典追问链

**追问 1（深度 2）：一句话说清两个 Hook 的执行时机差别？**

出题意图：热身题，确认「同在 DOM 更新后、分 paint 前后」这个基本坐标——答「同步 vs 异步」不算错但要能落到帧上。
...
- 加分项：类组件的对应物：componentDidMount/Update 属于 layout 档（paint 前同步），所以迁移到 Hooks 时老代码里的测量逻辑要显式换成 useLayoutEffect。

**追问 2（深度 3）：在 useLayoutEffect 里 setState 会发生什么？为什么它不闪烁反而成了防闪烁手段？**

出题意图：考「同步重渲染」的机制：能说清 paint 前的二次 render/commit，才算理解防闪烁的原理而不是背结论。
...
- 加分项：推论：在 useLayoutEffect 里写无条件 setState（比如 setState(Date.now())）而依赖数组又包含该状态，就是 paint 前无限循环的直接配方。

**追问 3（深度 3）：useEffect 一定在「下一帧绘制之后」执行吗？**

出题意图：辨析题：区分「不阻塞 paint 的保证」与「精确的帧定时」——把 useEffect 当 rAF 用的人会在这里翻车。
...
- 加分项：浏览器一帧的顺序是 JS → rAF → Style/Layout → Paint；passive effects 不在这条固定链上，这就是两者定性差异的根源。

**追问 4（深度 4）：测量为什么必须「读布局 + 写回」放在同一个 useLayoutEffect 里？拆成两个行不行？**

出题意图：把浏览器强制同步布局与 React 执行档位缝在一起——这是前端两条主线的交汇点，答得出的是调过真实布局的人。
...
- 加分项：量多个元素时遵守读写分离：先把所有几何批量读完（首个读触发唯一一次布局），再批量写回——否则在 paint 前自己制造布局抖动。

**追问 5（深度 4）：React 19 为什么移除了 SSR 下 useLayoutEffect 的警告？警告没了，风险也没了吗？**

出题意图：版本行为题，考「警告与语义」的分离——以为是行为变化的人，说明没真正理解服务端为什么不执行 effect。
...
- 加分项：更稳的做法是把「首屏就必须正确」的定位交给 CSS（固定定位、anchor 定位），把 JS 测量留给水合后的交互态——服务端能给的布局不要欠着客户端。

## 延伸阅读

- 下一站：《Hook 为什么不能写在条件语句里？》——两个 Hook 凭什么按调用顺序对号入座：memoizedState 链与错位的实际后果。
- 下一站：《Fiber 为什么能让渲染可中断？》——本篇上游的完整机制：render/commit 两阶段怎么切、副作用 flags 从哪来、lane 优先级怎么排。
