...
*类型：knowledge ｜ 难度：进阶 ｜ 标签：React、setState、批处理、调度 ｜ 更新：2026-09-10*

**调用 setState 只做两件事：把 update **入队**、把组件**标记**为待更新——它不改任何变量的值，而是「请求一次重新渲染」。真正的渲染由**调度器**在本轮事件处理全部结束后合并执行，所以调用后立刻读 state 一定是旧值。React 18 起**自动批处理**覆盖所有场景（setTimeout/Promise/原生事件里同样合并），「立刻刷出去」的唯一逃生舱是 `flushSync`。**

## 前置篇说明

- 前置篇：《连续 setState 为什么只加一次：批处理》（hooks/hooks-render）
- 那篇从使用者视角讲「连续 setState 为什么只 +1」（闭包快照 + 交互演示）；本篇下潜一层，讲 React 拿到这次调用之后在内部做了什么。两篇视角互补，分界在「行为 vs 机制」。
...
- 输出 1：三次 update 都是「直接替换为 1」：队列里存的是目标值，最后一次替换决定结果——替换执行多少遍都与渲染次数无关。
- 输出 3：三次 update 都是函数：重放时从 baseState 出发依次计算，0 → 1 → 2 → 3，每一步吃上一步的返回值——存「操作」才保得住连续累加。
...
引擎层面的动作是：从触发更新的 Fiber 节点向上找到 root，把这次 update 的优先级记到 root 的待处理标记上（`pendingLanes`，机制见「Fiber 为什么能让渲染可中断？」篇）。调度器拿到请求后合并去重——同一轮里第二次 setState 发现已有待处理渲染，就只追加队列、不再重复请求。下图是一次调用的完整旅程：

```text
一次 setState 的旅程 / update flow
[setState(update)] → [update 入队] → [标记 Fiber 待更新]
      → [调度器合并请求]（本轮只发一次）
      → [render 阶段重放队列]（事件处理结束后）
      → [commit 更新 DOM]
```
...
反例 vs 正例（updater 使用纪律 / pure updater）：

反例——updater 不纯 + 滥用逃生舱：副作用翻倍、每行 setState 都触发一次完整渲染：

```typescript
// 副作用进 updater + 无脑 flushSync
setCount(c => {
...
正例——副作用移到 useEffect；flushSync 单次、点状使用，用完即走：

```typescript
// updater 保持纯：只算，不做副作用
setCount(c => c + 1);
...
反例——把 setState 当成立刻改值——它是「请求一次调度」：
...
```typescript
setCount((c) => c + 1); // 函数式更新读到最新值
useEffect(() => { console.log(count); }, [count]); // 落地后再读
```

## 追问链

**追问 1（深度 2）：setState 之后立刻 console.log，为什么还是旧值？怎么拿到更新后的值？**

出题意图：热身题，筛掉「setState 是异步的」这种半对半错的表述——真正的答案是调度，不是异步。

setState 是同步调用、异步生效的调度行为：它只把 update 入队并标记组件待更新，状态在本轮事件处理结束后的那次渲染才生效——它不是 Promise 意义上的异步 API。要基于最新值连续更新，用函数式写法 setCount(c => c + 1)；要拿「渲染后」的时机，用 useEffect（依赖该状态）或 flushSync 强制同步刷新后再读。

- 加分项：类组件 setState 的第二参数回调与 componentDidUpdate 时机一致——commit 完成后同步执行，早于 passive 的 useEffect。

**追问 2（深度 3）：setCount(count + 1) 写三次只 +1，setCount(c => c + 1) 写三次能 +3——队列里到底发生了什么？**

出题意图：考批处理的实现层：答出「队列存操作、渲染期重放」才算理解机制，只答「闭包」说明停留在现象。
...
- 加分项：react.dev 的课后练习 getFinalState 就是这个重放算法——for 循环里 typeof update === 'function' ? update(state) : update，值得亲手写一遍。

**追问 3（深度 3）：React 18 的自动批处理有没有覆盖不到的场景？**

出题意图：考边界：能说出 flushSync 与「批处理不跨多次有意事件」才算完整掌握。

覆盖不到的只有两类。一是 flushSync 包裹的更新，立即同步渲染；二是批处理不跨「多次有意事件」——第一次 click 和第二次 click 各自合并，不会跨事件攒批，这保证了「第一下禁用表单、第二下就不会再提交」这类时序语义可预测。除此之外事件、setTimeout、Promise、原生监听里的更新全部默认批处理。

- 加分项：flushSync 内部是「同步刷完当前队列再继续」，每次调用都强制一次完整渲染——滥用等于手动退回 React 17 之前的渲染粒度。

**追问 4（深度 4）：setState 的「异步生效」和 microtask 有关系吗？渲染请求排进了哪个队列？**

出题意图：把 React 调度与事件循环区分开——能分清「谁管队列、谁管渲染」的才是理解过调度层的人。
...
- 加分项：离散事件（click）与连续事件（scroll）的更新优先级不同——这条线索通向 Fiber 的 lane 模型，见「Fiber 为什么能让渲染可中断？」。

**追问 5（深度 4）：为什么 updater 函数必须是纯的？StrictMode 为什么要把 updater 跑两遍？**

出题意图：压轴题，为 Fiber 的可中断渲染埋线——答出「渲染期可能重放、可能丢弃重来」就直接通向下一篇。
...
- 加分项：这个前提在并发渲染里是硬约束：render 阶段被高优先级插队后，半成品直接作废重来——不纯的 updater 会在用户无感知的情况下重复执行。

## 延伸阅读

- 下一站：《Fiber 为什么能让渲染可中断？》——排队之后渲染怎么走：render/commit 两阶段、双缓存与 lane 优先级——本篇的下一问。
- 下一站：《用 index 做 key 为什么会状态错位？》——render 阶段的核心对账算法：key 如何决定节点的复用、移动与重建。
