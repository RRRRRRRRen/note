...
*类型：practice ｜ 难度：进阶 ｜ 标签：yudao、Maven、POM、BOM、依赖管理*

**yudao 项目的 Maven 实践覆盖四个层次：启动方式差异（mvn spring-boot:run vs java -jar 的类路径与依赖来源不同）、常用命令与插件（围绕生命周期 phase 执行）、BOM 版本清单（只定版本不引包，实现技术栈对齐）、POM 项目模型（继承统一规范、聚合统一构建）。核心心智：版本集中在 dependencyManagement，插件版本显式固定，变更后用 effective-pom 验证。**
...
# -Dspring-boot.run.profiles 指定 Spring profile
...
### IDEA 启动 Spring Boot 项目的原理

IDEA 没有创造新机制，本质是帮你“生成并执行启动命令”。

方式一：Application / Spring Boot 配置——IDEA 先编译模块（等价增量编译），按模块依赖拼接 classpath，直接调用 JVM 启动 `main`。最接近“手工 `java -cp ...` + `main`”。

方式二：Maven 配置——IDEA 调用 Maven 可执行程序，继续执行 `spring-boot:run`。实际行为受插件配置和 Maven 环境影响更大。

实践建议：开发调试优先 Application，启动更快、调试体验更好；验证构建一致性时用 `mvn clean package` + `java -jar`；CI 以命令行为准，避免只在 IDE 中“看起来能跑”。
...
- 常用生命周期：`clean`、`default`、`site`，最常用的是 `default`。
...
BOM（Bill of Materials）是“版本清单”，不负责引包，负责定版本：目标是“同一技术栈版本对齐”，核心收益是“升级集中化”和“冲突可控化”，适合多模块、多人协作、长期维护项目。
...
应用场景：多模块工程统一依赖版本、多仓库项目保持技术栈一致、团队做“批量可回滚”的版本升级。
...
POM（Project Object Model）是 Maven 项目的“单一事实源”：定义项目坐标（`groupId`、`artifactId`、`version`）、依赖、构建方式与规则（编码、JDK、插件版本、仓库等）。
...
继承（parent）：子模块继承父 POM 的配置，典型继承内容为属性、依赖管理、插件管理、仓库配置，适合“统一规范”。

聚合（modules）：父工程聚合多个子模块统一构建，父工程 `packaging` 通常为 `pom`，适合“多模块一键构建”。
...
`properties` 用来抽取重复配置，提升一致性；依赖和插件可通过 `${xxx}` 复用：
...
`profiles` 是“按条件激活配置”，不是“运行时配置中心”。常见激活方式有 `-P` 指定、JDK 条件、文件存在、系统属性；典型用途是不同环境仓库地址、资源过滤参数、插件行为。`settings.xml` 适合放镜像与凭据，避免写进项目仓库。
